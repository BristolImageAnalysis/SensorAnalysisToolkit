...
int main(int argc, const char** argv){
	...
	//mask
	stk::ImageMask<unsigned short> myMask;
	...
	//Image to hold mask
	std::shared_ptr<stk::Image<unsigned short> > myMaskImage ( new stk::Image<unsigned short>(4096,4096) );
	...
	myMask.MaskImage(myVarianceImage, myMaskImage, 500);
	...
	//set up hist
		...
		myImageHistogram.SetTitle("Mask");
		...
		myImageHistogram.Generate2DHistogram( myMaskImage );
		...
		std::cout << "Time for Histogram Generation " << time_span_5.count() << " seconds." << std::endl;
		return 1;
